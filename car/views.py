...
def search(request):
    if request.method=='GET':
        ...
        # default range effectively disables the price filter when no slider value was submitted
        slider_range = request.GET.get('slider') or "0,999999999"
...
# Generate a one-time-use OTP and deliver it to the requesting client.
# No SMS gateway is configured for this project, so the OTP is surfaced
# back to the user via the messages framework instead of being texted.
